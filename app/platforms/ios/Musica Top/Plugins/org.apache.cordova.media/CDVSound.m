...
#pragma unused(callbackId)
        NSString *mediaId;
        if (command != nil) {
            mediaId = [command argumentAtIndex:0];
            self.currentMediaId = mediaId;
        } else {
            mediaId = self.currentMediaId;
        }
...
//            [self pausePlayingAudio:nil];
        ...
        case UIEventSubtypeRemoteControlNextTrack:
            NSLog(@"NEXT!!!");
            if (nextString) {
                [self.commandDelegate evalJs:nextString];
            }
            break;
        case UIEventSubtypeRemoteControlPreviousTrack:
            NSLog(@"PREV!!!");
            if (prevString) {
                [self.commandDelegate evalJs:prevString];
            }
            break;
        default:
            break;
...
- (void)setInfoCenterInfo:(NSDictionary *)songdata AndDuration:(float)duration andPosition:(float)pos {
    ...
    [self.commandDelegate runInBackground:^{
        ...
        if (setInfoCenterInfoError) {
            ...
            NSString *albumart = [songdata objectForKey:@"albumart"];
            MPMediaItemArtwork *albumArt = [[MPMediaItemArtwork alloc] initWithImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:albumart]]]];
